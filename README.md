...
independent projects work, while ancestor workspace lockfiles are not consumed.
...
|-------|------|---------|-------------|
| `manifest_path` | string | `"package.json"` | Path to a `package.json` carrying the `packageManager` field. It must be relative, must not contain `..`, a newline, or a carriage return, and needs an existing tracked non-symlink `pnpm-lock.yaml` beside it. A leading `./` is normalized away so the value matches what the pull-request files API reports |
