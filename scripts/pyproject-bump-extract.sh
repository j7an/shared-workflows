...
# Sets _poetry_inline_version and _poetry_inline_skeleton (version substituted).
parse_poetry_inline() {
  local value="$1" inner
  _poetry_inline_version=""; _poetry_inline_skeleton=""
  if [[ "$value" =~ ^[[:space:]]*\{(.*)\}[[:space:]]*$ ]]; then
    inner="${BASH_REMATCH[1]}"
    if [[ "$inner" =~ version[[:space:]]*=[[:space:]]*\"([^\"]*)\" ]]; then
      _poetry_inline_version="${BASH_REMATCH[1]}"
      _poetry_inline_skeleton=$(printf '%s' "$inner" | sed -E 's/version[[:space:]]*=[[:space:]]*"[^"]*"/version=__VER__/')
      return 0
    fi
  fi
  return 1
...
emit_bump() {
  local name="$1" plus_spec="$2" minus_spec="$3" target
...
while IFS= read -r line; do
  ...
  # Closing `]` of an array on a changed line = reformatting → disqualify.
  if [[ "$content" =~ ^[[:space:]]*\] ]]; then
    if [ "$prefix" = "+" ] || [ "$prefix" = "-" ]; then verdict="disqualified"; fi
    continue
  fi
