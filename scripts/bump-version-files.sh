#!/usr/bin/env bash
# bump-version-files.sh — apply version bumps from a version-bump config
#
# Reads the config (default .version-bump.json) and writes the supplied version into each
...
#
# Config path contract: relative to the repository root, no '..' segment,
# ends in .json, not a symlink, and canonically inside GITHUB_WORKSPACE
# (default $PWD). Entry paths inside the config are always relative to the
# repository root (the working directory), not to the config's location.
#
# Exit codes:
#   0 — at least one entry was modified (caller should commit)
#   1 — config error: unsafe config path, malformed JSON, missing files
#       array, schema violation. Caller MUST fail the workflow.
...
bump_entry() {
  ...
  if [ -n "$path_expr" ] && ! validate_path_expr "$path_expr"; then
    printf '::error file=%s::Entry for %q has invalid path_expr %q\n' \
      "$config" "$file_path" "$path_expr" >&2
...
# Shape checks run before the manifest is truncated; the containment check
# below runs before the config is read. Control characters are refused so
# the value cannot forge workflow commands in diagnostics.
case "$config" in
  *[[:cntrl:]]*)
    echo "::error::version-bump config contains a control character" >&2
    exit 1 ;;
  /*)
    echo "::error::version-bump config '$config' is absolute; it must be relative to the repository root" >&2
    exit 1 ;;
esac
case "/$config/" in
  */../*)
    echo "::error::version-bump config '$config' contains a '..' path segment" >&2
    exit 1 ;;
esac
case "$config" in
  *.json) ;;
  *)
    echo "::error::version-bump config '$config' must end in .json" >&2
    exit 1 ;;
esac
# pwd -P below resolves directory symlinks; this closes the file itself.
if [ -L "$config" ]; then
  echo "::error::version-bump config '$config' is a symlink" >&2
  exit 1
fi
...
case "$config_dir/" in
  "$ws_real"/*) ;;
  *)
    echo "::error::version-bump config '$config' resolves outside the checkout" >&2
    exit 1 ;;
esac
...
# the config has multiple entries against it (see test
